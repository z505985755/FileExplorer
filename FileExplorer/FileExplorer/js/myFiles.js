...
function hashChange(path, search, order) {
    ...
    order = _order;
    $.getJSON(
        "ashx/getUserFolder.ashx?_=" + Math.random(),
...
function showFolder(data, path) {
    ...
    for (var i = 0; i < data.files.length; i++) {
        ...
        if (imei == "image") {//如果是图片类型的,则直接预览该图片
            str += "<div class='fileicon'><img src='../" + path + "/" + data.files[i].name + ".preview'/></div>";
        }
        else {
    ...
    uploader.on('uploadSuccess', function (file, response) {
        ...
        if (response.hasError === true) {
            ...
            if (response.chunked === true) {
                ...
                $.post("ashx/MergeFile.ashx", { fileName: response.fileName, path: response.path },
                function (data) {
                    data = $.parseJSON(data);
                    if (data.hasError) {
                        alert('文件合并失败！');
...
function downloadFiles() {
    ...
    iframe.src = "ashx/downloadFiles.ashx?names=" + escape(names) + "&path=" + escape(path);
}
//快捷键注册
